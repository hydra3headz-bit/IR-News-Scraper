...
def find_ir_page(ticker):
    ...
    # 1. Hardcoded Map for High-Priority Stocks
    ticker_map = {
    ...
    # 2. Yahoo Finance Profile (Most reliable way to get the OFFICIAL domain)
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # Look for the website link on the profile page
            for a in soup.find_all('a'):
                href = a.get('href', '')
                if 'http' in href and not any(x in href for x in ['yahoo.com', 'google.com', 'twitter.com']):
                    # This is likely the company's main site
                    base_url = href.rstrip('/')
                    logger.info(f"Yahoo found base domain: {base_url}")
                    
                    # Try common IR paths on this domain
                    ...
                    for path in ir_paths:
                        ...
                        try:
                            ...
                            continue
                    return base_url # Return base if no IR path found
    except Exception as e:
        logger.error(f"Yahoo domain lookup failed for {ticker}: {e}")

    # 3. Direct Guesses (as a backup)
    ...
    for url in guesses:
        try:
            if requests.get(url, timeout=3, verify=False).status_code < 400:
                return url
        except:
            continue

    # 4. Final Fail-safe Search
    try:
            ...
            for item in items:
                ...
                if dt >= cutoff_date:
                    results.append({
                        ...
                        'headline': f"[Yahoo/Aggregate] {title[:150]}",
                        'link': link
                    })
    except: pass
    return results
            ...
            for entry in entries:
                ...
                if dt >= cutoff_date:
                    results.append({
                        'ticker': ticker,
                        'date': dt.strftime("%Y-%m-%d"),
                        'headline': f"[Reddit/WSB] {title[:150]}",
                        'link': link
                    })
    except: pass
    return results
                ...
                if dt >= cutoff_date:
                    ...
                    for _ in range(6):
                        ...
                        for a in a_tags:
                            h = a.get_text().strip()
                            href = a.get('href', '')
                            if href and len(h) > 12:
                                link, headline = href, h
                                break
                        if link: break
                        curr = curr.parent
                    if link and headline:
                        ...
                        results.append({
                            'ticker': ticker,
                            'date': dt.strftime("%Y-%m-%d"),
                            'headline': f"[Official IR] {headline.replace('\n', ' ').strip()[:150]}",
                            'link': link
                        })
    except: pass
    return results
...
def get_news(url, ticker, days_lookback=7, recursive=True):
    ...
    seen_headlines = set()
    for item in results:
        h_norm = item['headline'].lower()
        if h_norm not in seen_headlines:
